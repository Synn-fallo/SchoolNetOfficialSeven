...
export default function EtablissementRequestsList() {
  ...
  const handleCancel = async (requestId: string) => {
    if (!confirm('Êtes-vous sûr de vouloir annuler cette demande de création d\'établissement ?')) return;
  ...
  if (requests.length === 0) {
    return (
      <div className="flex items-center justify-center py-10">
        <p className="text-sm text-slate-400">Aucune demande d'établissement</p>
      </div>
    );
  }
